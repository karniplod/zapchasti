...
async def create_standalone(
    ...
    sku = (await session.execute(text(
        "SELECT 'P-' || lpad(nextval('standalone_part_seq')::text, 4, '0')"
    ))).scalar_one()
    ...
    part_id = (await session.execute(text("""
        ...
        RETURNING id
    """), {"sku": sku, "cat": category_id, "name": name.strip(), "oem": oem,
           "cond": condition, "note": condition_note, "price": price,
           "loc": location, "st": "in_stock" if files else "draft",
           "pub": bool(files and price), "src": source})).scalar_one()
...
async def standalone_list(session: AsyncSession = Depends(get_session),
                          user=Depends(require_role("manager"))):
