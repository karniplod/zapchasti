...
BODY_TYPES = ["седан", "хэтчбек", "универсал", "лифтбек", "купе", "кабриолет",
...
async def add_brand(payload: BrandIn,
    ...
    existing = (await session.execute(
        text("SELECT id, name FROM brands WHERE slug = :s"), {"s": slug}
    )).first()
    if existing:
...
async def add_model(payload: ModelIn,
                    user=Depends(require_role("dismantler")),
                    session: AsyncSession = Depends(get_session)):
    name = payload.name.strip()
    slug = slugify(name)
    ...
    """), {"b": payload.brand_id, "s": slug})).first()
    if existing:
        return {"id": existing.id, "name": existing.name, "existed": True}
    ...
    row = (await session.execute(text("""
        ...
        RETURNING id, name
    """), {"b": payload.brand_id, "n": name, "s": slug})).first()
    await session.commit()
    return {"id": row.id, "name": row.name, "existed": False}


@router.post("/generations", status_code=201)
async def add_generation(payload: GenerationIn,
                         user=Depends(require_role("dismantler")),
                         session: AsyncSession = Depends(get_session)):
    ...
    """), {"m": payload.model_id, "n": name})).first()
    if existing:
        return {"id": existing.id, "name": existing.name, "existed": True}
    ...
    row = (await session.execute(text("""
        ...
        RETURNING id, name
    """), {"m": payload.model_id, "n": name, "b": payload.body_type,
           "yf": payload.year_from, "yt": payload.year_to})).first()
    await session.commit()
...
async def add_modification(payload: ModificationIn,
                           user=Depends(require_role("dismantler")),
                           session: AsyncSession = Depends(get_session)):
    row = (await session.execute(text("""
        ...
        RETURNING id
    """), {"g": payload.generation_id, "ec": payload.engine_code,
           "ev": payload.engine_volume, "f": payload.fuel,
           "p": payload.power_hp, "t": payload.transmission,
           "d": payload.drive})).first()
    await session.commit()
    return {"id": row.id}
...
@router.get("/review")
async def review_queue(user=Depends(require_role("manager")),
                       session: AsyncSession = Depends(get_session)):
...
async def approve(generation_id: int,
...
async def merge(generation_id: int, into_id: int,
                user=Depends(require_role("manager")),
                session: AsyncSession = Depends(get_session)):
    ...
    moved = (await session.execute(text("""
        ...
        RETURNING id
    """), {"into": into_id, "from_id": generation_id})).rowcount
