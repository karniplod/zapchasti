...
CONDITION_LABELS = {
    "A": "Отличное", "B": "Рабочее",
    "C": "С дефектом", "D": "Под восстановление",
}
...
async def part_page(sku: str, request: Request,
                    session: AsyncSession = Depends(get_session)):
    """Сюда ведёт QR с этикетки — и для кладовщика, и для покупателя."""
    part = (await session.execute(text("""
        SELECT p.id, p.sku, p.name, p.oem_number, p.condition::text AS condition,
         ...
         WHERE p.sku = :sku AND p.published
    """), {"sku": sku})).first()

    if not part:
        raise HTTPException(404, "Деталь не найдена или снята с продажи")

    photos = [r.path for r in await session.execute(text("""
    ...
    fits = []
    if part.oem_number:
        fits = [dict(r._mapping) for r in await session.execute(text("""
    ...
    return templates.TemplateResponse("part.html", {
        "request": request,
...
async def catalog_vin(payload: VinSearch, session: AsyncSession = Depends(get_session)):
    wmi_map = {r.code: r.manufacturer for r in
               await session.execute(text("SELECT code, manufacturer FROM wmi"))}
    ...
    result = {
        "vin": info.vin, "wmi": info.wmi, "vds": info.vds,
        "country": info.country, "manufacturer": info.manufacturer,
        "year": info.year, "warnings": info.warnings,
    }
    ...
    if hit:
        chain = (await session.execute(text("""
            SELECT b.name AS brand, m.name AS model,
              ...
              JOIN brands b      ON b.id = m.brand_id
             WHERE mo.id = :id
        """), {"id": hit.modification_id})).first()

        if chain:
            result.update(resolution="exact", confidence=hit.confidence,
                          **dict(chain._mapping))
    ...
    if info.manufacturer:
        candidates = [dict(r._mapping) for r in await session.execute(text("""
        ...
        if candidates:
            ...
            await log_query(session, info, "brand_year", None,
...
async def count_fitting(session: AsyncSession, generation_id: int,
                        modification_id: int | None) -> int:
    return (await session.execute(text(f"""
        SELECT count(*) FROM parts p LEFT JOIN donors d ON d.id = p.donor_id
         WHERE {FITS_CLAUSE}
    """), {"gen": generation_id, "mod": modification_id})).scalar_one()


async def log_query(session: AsyncSession, info, resolution: str,
                    generation_id: int | None, count: int):
    await session.execute(text("""
        INSERT INTO vin_queries (vin, wmi, vds, resolution, generation_id, results_count)
        VALUES (:vin, :wmi, :vds, :res, :gen, :cnt)
    """), {"vin": info.vin, "wmi": info.wmi, "vds": info.vds,
           "res": resolution, "gen": generation_id, "cnt": count})
...
async def catalog_parts(
    ...
    if generation_id:
        where = [FITS_CLAUSE]
    if brand_id:
        where.append("b.id = :brand"); params["brand"] = brand_id
    if model_id:
        where.append("m.id = :model"); params["model"] = model_id
    if category_id:
    ...
    if price_min is not None:
        where.append("p.price >= :pmin"); params["pmin"] = price_min
    if price_max is not None:
        where.append("p.price <= :pmax"); params["pmax"] = price_max
    if q:
        # Каталожный номер ищем по нормализованной форме, название — по триграммам
        where.append("(p.name ILIKE '%' || :q || '%' OR p.oem_number = :oem)")
        params["q"] = q.strip()
    ...
    return {"total": total, "page": page, "pages": -(-total // PAGE_SIZE), "items": items}


@router.get("/api/catalog/facets")
async def catalog_facets(generation_id: int | None = None,
                         modification_id: int | None = None,
                         session: AsyncSession = Depends(get_session)):
    ...
    cats = await session.execute(text(f"""
    ...
    price = (await session.execute(text(f"""
    ...
    return {
        ...
        "conditions": [{**dict(r._mapping),
                        "label": CONDITION_LABELS.get(r.condition)} for r in conds],
