# Автодонор — каталог б/у автозапчастей
...
│   ├── customer_auth.py      вход покупателя: телефон и пароль
...
│   │   ├── pages.py          контакты, доставка, заявки, sitemap
│   │   ├── shop.py           корзина, заказ, оплата, личный кабинет
│   │   └── reference.py      справочник, добавление на бегу
│   ├── services/
│   │   ├── images.py         ресайз, webp, миниатюры
│   │   ├── geo.py            город покупателя по IP (GeoLite2)
│   │   ├── oem.py            подсказка каталожного номера, веса, точность
│   │   └── parsers/          внешние источники номеров (выключены по умолчанию)
│   └── scripts/
...
│       ├── rebuild_photos.py      пережать снимки, залитые до обработчика
│       ├── check_search.py         проверить ключ поисковика для подсказки
...
│   ├── contacts.html            адреса филиалов и форма заявки
│   ├── delivery.html            доставка и самовывоз
│   ├── shop/                    корзина, вход, кабинет, заказ
│   ├── _city.html               выбор города в шапке
...
└── static/
    ├── admin.css            бэкенд: тёмная тема целиком
    └── css/
        ├── core.css         ядро витрины: токены, кнопки, поля, плитка
        ├── chrome.css       шапка, меню, город, подвал
        ├── home.css, catalog.css, part.css, error.css   по странице
        ├── pages.css        контакты и доставка
        └── shop.css         корзина, кабинет, заказы
```

Стили витрины разложены по слоям: `core.css` — общий словарь, `chrome.css` —
то, что есть на каждой странице, дальше файл конкретной страницы. Подключает
их `_base.html` в этом порядке, поэтому страница может уточнить ядро, а ядро
о страницах не знает. Правило: селектор, который встречается только на одной
странице, живёт в её файле.
...
**Города и филиалы.** Машина принимается в конкретный филиал, снятые
с неё детали наследуют его автоматически — отдельно выбирать филиал
разборщику не нужно. В каталоге город виден как фильтр, на карточке
детали — как адрес, куда за ней ехать. Список филиалов лежит в таблице
`branches`; чтобы добавить новый, достаточно вставить туда строку.

**Определение города по IP — подсказка, а не ограничение.** При первом
заходе город подставляется из локальной базы MaxMind, выбранный руками
сохраняется в cookie на год и всегда важнее автоматики. Базы в репозитории
нет: нужно скачать бесплатную `GeoLite2-City.mmdb`
(https://dev.maxmind.com/geoip/geolite2-free-geolocation-data, требуется
регистрация) и положить в `data/GeoLite2-City.mmdb` — путь меняется
настройкой `GEOIP_DB`. Без файла определение просто выключено, каталог
работает как обычно. На localhost город не определяется никогда: у
приватных адресов его нет.

**Кабинет покупателя держит деталь, а не корзину.** Деталь штучная,
поэтому оформленный заказ сразу переводит её в `reserved` и убирает
с витрины — двое не купят один бампер. Отмена заказа возвращает деталь
в продажу, но только если её не успели продать кому-то ещё.

**Корзина живёт в куке до входа** и привязывается к покупателю в момент
регистрации: иначе человек выбирает деталь, идёт заводить кабинет
и возвращается к пустой корзине.
...
1. Онлайн-оплата: способов не подключено. Заказ оформляется и держит
   деталь, платит человек при получении, менеджер отмечает оплату
   в бэкенде. Провайдер добавляется строкой в `PAYMENT_METHODS`
   и обработчиком в `shop.pay()` — схема и шаблоны уже под это готовы
2. Выгрузка на Авито и Дром
3. ЧПУ-разделы каталога
4. Alembic-миграции — схема накатывается файлами из `sql/`
5. Тесты
6. Таблица `wmi` пуста: VIN-декодер не называет завод-изготовитель,
   первую машину каждой модели приёмщик заводит вслепую
7. Фото при разборе и приёмке отдельной детали кладутся оригиналом,
   без webp и миниатюр (`dismantle.py`, `stock.py`). Уже загруженные
   лечатся через `python -m app.scripts.rebuild_photos --apply`
