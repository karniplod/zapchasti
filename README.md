...
1. Корзина и оформление заказа. Пока вместо них заявка: покупатель
   оставляет телефон на карточке детали или в контактах, заявка падает
   в `leads` и считается в сводке бэкенда
2. Онлайн-оплата
3. Выгрузка на Авито и Дром
4. ЧПУ-разделы каталога
5. Alembic-миграции — схема накатывается файлами из `sql/`
6. Тесты
7. Таблица `wmi` пуста: VIN-декодер не называет завод-изготовитель,
   первую машину каждой модели приёмщик заводит вслепую
8. Фото при разборе и приёмке отдельной детали кладутся оригиналом,
   без webp и миниатюр (`dismantle.py`, `stock.py`). Уже загруженные
   лечатся через `python -m app.scripts.rebuild_photos --apply`
9. Кроссы OEM пусты: таблицы созданы, но выгрузку TecDoc
